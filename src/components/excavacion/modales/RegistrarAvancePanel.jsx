...
const RegistrarAvancePanel = ({ panelId, onClose, onSuccess }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Crear objeto de avance con el ID del panel y la URL de la imagen
      const avanceData = {
        ...data,
        id_panel: panelId,
        imagenurl: imageUrl
      ...
      if (mutate) {
        ...
        mutate(avanceData, {
          ...
          onError: (error) => {
            ...
            setServerError(error.response?.data?.error || 'Error al registrar el avance');
          },
          onSettled: () => {
        ...
        await avance_panelService.create(avanceData);
        toast.success('Avance registrado correctamente');
        onClose();
        if (onSuccess) onSuccess();
        setIsUploading(false);
  ...
  return (
    <ModalContainer>
      <Modal>
        <FormTitle>Registrar Avance de Panel <X onClick={onClose} className="cursor-pointer" /></FormTitle>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex flex-row justify-end gap-4">
            <button
              type="button"
              className="bg-slate-100 text-slate-500 py-3 px-6 rounded-lg"
              onClick={onClose}
            >
              Cancelar
            </button>
